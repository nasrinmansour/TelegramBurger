...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ArrayList<TLRPC.Dialog> dialogsFavs = new ArrayList();
    public ArrayList<TLRPC.Dialog> dialogsGroups = new ArrayList();
    public ArrayList<TLRPC.Dialog> dialogsGroupsAll = new ArrayList();
    ...
    public MessagesController() {
        ...
        maxEditTime = preferences.getInt("maxEditTime", 172800);
        ...
        if (UserConfig.isClientActivated()) {
            if (ConnectionsManager.getInstance().getPauseTime() == 0 && ApplicationLoader.isScreenOn && !ApplicationLoader.mainInterfacePaused) {
                if (statusSettingState != 1 && (lastStatusUpdateTime == 0 || Math.abs(System.currentTimeMillis() - lastStatusUpdateTime) >= 55000 || offlineSent)) {
                    ...
                    if(!SepehrSettingsHelper.getGhostModeActivationState()) {
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        for (int a = 0; a < dialogs.size(); a++) {
                            ...
                            if ((MessagesController.this.getEncryptedChat(Integer.valueOf(high_id)) instanceof TLRPC.TL_encryptedChat)) {
                                MessagesController.this.dialogsUsers.add(d);
                            }
                            if (Favourite.isFavourite(Long.valueOf(d.id))) {
                                MessagesController.this.dialogsFavs.add(d);
                            }
                            TLRPC.User localUser = MessagesController.this.getUser(Integer.valueOf((int) d.id));
                            if (localUser != null) {
                                if (localUser.bot) {
                                    MessagesController.this.dialogsBots.add(d);
                                } else {
                                    MessagesController.this.dialogsUsers.add(d);
                                }
                            }
                            if ((d instanceof TLRPC.TL_dialogChannel)) {
                                int k = (int) d.id;
                                TLRPC.Chat localChat = MessagesController.this.getChat(Integer.valueOf(-k));
                                if ((localChat != null) && (((localChat.megagroup) && (localChat.editor)) || (localChat.creator))) {
                                    MessagesController.this.dialogsGroupsOnly.add(d);
                                }
                                if (localChat != null) {
                                    if (localChat.megagroup) {
                                        MessagesController.this.dialogsMegaGroups.add(d);
                                        MessagesController.this.dialogsGroupsAll.add(d);
                                    } else {
                                        MessagesController.this.dialogsChannels.add(d);
                                    }
                                }
                            }
                        }
    ...
    public void deleteUserFromChat(final int chat_id, final TLRPC.User user, final TLRPC.ChatFull info, final boolean deleteOnLeave) {
        if (user == null) {
            return;
        }
        if (chat_id > 0) {
            ...
            ConnectionsManager.getInstance().sendRequest(request, new RequestDelegate() {
                @Override
                public void run(TLObject response, TLRPC.TL_error error) {
                    if (user.id == UserConfig.getClientUserId() && deleteOnLeave) {
                        AndroidUtilities.runOnUIThread(new Runnable() {
                            @Override
                            public void run() {
