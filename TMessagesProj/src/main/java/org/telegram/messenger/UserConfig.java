...
public class UserConfig {
    ...
    public static String sepehrPasscodeHash = "";
    public static byte[] passcodeSalt = new byte[0];
    public static byte[] sepehrPasscodeSalt = new byte[0];
    ...
    public static boolean useFingerprintForSepehr = true;
    ...
    public static void saveConfig(boolean withFile, File oldFile) {
        synchronized (sync) {
            try {
                ...
                editor.putString("passcodeHashSepehr", sepehrPasscodeHash);
                editor.putString("passcodeSalt", passcodeSalt.length > 0 ? Base64.encodeToString(passcodeSalt, Base64.DEFAULT) : "");
                editor.putString("passcodeSaltSepehr", sepehrPasscodeSalt.length > 0 ? Base64.encodeToString(sepehrPasscodeSalt, Base64.DEFAULT) : "");
    ...
    public static void loadConfig() {
        synchronized (sync) {
            ...
            if (configFile.exists()) {
                ...
                sepehrPasscodeHash = preferences.getString("passcodeHashSepehr", "");
                ...
                String passcodeSaltSepehrString = preferences.getString("passcodeSaltSepehr", "");
                if (passcodeSaltSepehrString.length() > 0) {
                    sepehrPasscodeSalt = Base64.decode(passcodeSaltSepehrString, Base64.DEFAULT);
                } else {
                    sepehrPasscodeSalt = new byte[0];
                }
            }
        }
    }
    ...
    public static boolean checkSepehrPasscode(String passcode) {
        if (sepehrPasscodeSalt.length == 0) {
            boolean result = Utilities.MD5(passcode).equals(sepehrPasscodeHash);
            if (result) {
                try {
                    sepehrPasscodeSalt = new byte[16];
                    Utilities.random.nextBytes(sepehrPasscodeSalt);
                    byte[] passcodeBytes = passcode.getBytes("UTF-8");
                    byte[] bytes = new byte[32 + passcodeBytes.length];
                    System.arraycopy(sepehrPasscodeSalt, 0, bytes, 0, 16);
                    System.arraycopy(passcodeBytes, 0, bytes, 16, passcodeBytes.length);
                    System.arraycopy(sepehrPasscodeSalt, 0, bytes, passcodeBytes.length + 16, 16);
                    sepehrPasscodeHash = Utilities.bytesToHex(Utilities.computeSHA256(bytes, 0, bytes.length));
                    saveConfig(false);
                } catch (Exception e) {
                    FileLog.e("tmessages", e);
                }
            ...
            try {
                ...
                System.arraycopy(sepehrPasscodeSalt, 0, bytes, passcodeBytes.length + 16, 16);
                String hash = Utilities.bytesToHex(Utilities.computeSHA256(bytes, 0, bytes.length));
                return sepehrPasscodeHash.equals(hash);
            } catch (Exception e) {
                FileLog.e("tmessages", e);
            }
        }
