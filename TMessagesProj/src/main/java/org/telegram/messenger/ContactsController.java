...
public class ContactsController {
    ...
    public HashMap onlineUsersSectionsDict;
    public ArrayList onlineSortedUsersSectionsArray;
    ...
    public void checkAppAccount() {
        ...
        accounts = am.getAccountsByType("ir.javan.messenger");
        ...
        if (recreateAccount) {
            ...
            if (UserConfig.isClientActivated()) {
                try {
                    currentAccount = new Account("" + UserConfig.getClientUserId(), "ir.javan.messenger");
    ...
    public void deleteAllAppAccounts() {
        try {
            AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
            Account[] accounts = am.getAccountsByType("ir.javan.messenger");
            for (int a = 0; a < accounts.length; a++) {
                am.removeAccount(accounts[a], null, null);
            }
        } catch (Exception e) {
...
//        query.add(builder.build());
    ...
    public void initOnlineUsersSectionsDict(){
        ...
        Iterator iterator = this.usersSectionsDict.keySet().iterator();
        while (iterator.hasNext()) {
            ArrayList resultArrayList = new ArrayList();
            String key = (String) iterator.next();
            ArrayList<TLRPC.TL_contact> sectionArrayList = (ArrayList) this.usersSectionsDict.get(key);
            for (TLRPC.TL_contact c : sectionArrayList) {
                TLRPC.User user = MessagesController.getInstance().getUser(Integer.valueOf(c.user_id));
            ...
            {
                this.onlineUsersSectionsDict.put(key, resultArrayList);
