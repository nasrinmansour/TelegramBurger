...
import org.telegram.calendar.PersianCalendar;
...
//        languagesDict.put(localeInfo.shortName, localeInfo);
    ...
    public static String formatDateChat(long date) {
        ...
        int year = persianCalendar.getPersianYear();//rightNow.get(Calendar.YEAR);

        rightNow.setTimeInMillis(date * 1000);
        persianCalendar.setTimeInMillis(date * 1000);
        int dateYear = persianCalendar.getPersianYear();//rightNow.get(Calendar.YEAR);
...
//                String format = LocaleController.formatString("formatDateAtTime", R.string.formatDateAtTime, getInstance().formatterMonth.format(new Date(date * 1000)), getInstance().formatterDay.format(new Date(date * 1000)));
                PersianCalendar persianCalendar = new PersianCalendar();
                persianCalendar.setTimeInMillis(date*1000);
                String format = persianCalendar.getPersianDay()+" "+persianCalendar.getPersianMonthName();
                return String.format("%s %s", LocaleController.getString("LastSeenDate", R.string.LastSeenDate), format);
            } else {
                PersianCalendar persianCalendar = new PersianCalendar();
                persianCalendar.setTimeInMillis(date*1000);
                String format = LocaleController.formatString("formatDateAtTime", R.string.formatDateAtTime, /*getInstance().formatterYear.format(new Date(date * 1000))*/persianCalendar.getPersianShortDate(), getInstance().formatterDay.format(new Date(date * 1000)));
                return String.format("%s %s", LocaleController.getString("LastSeenDate", R.string.LastSeenDate), format);
            }
        } catch (Exception e) {
            FileLog.e("tmessages", e);
    ...
    public static String stringForMessageListDate(long date) {
        try {
            ...
            if (year != dateYear) {
                return String.valueOf(persianCalendar.getPersianShortDate());
//                return getInstance().formatterYear.format(new Date(date * 1000));
            } else {
