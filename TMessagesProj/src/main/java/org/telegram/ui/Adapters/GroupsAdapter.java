...
//        }
        ...
        if (p.isImageDirty() && Utility.isInternetAvailable(activity)) {
            activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    if (p.getId() == ((Group) h.icon.getTag()).getId()) {
                ...
                Thread t = new Thread() {
                    ...
                    public void run() {
                        activity.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                if (p.getId() == ((Group) h.bar.getTag()).getId()) {
                        ...
                        if (imageByte != null) {
                            ...
                            GroupDAO.updateGroup(activity, p, true);
                    ...
                    public Thread setGroup(Group p) {
                        this.p = p;
                        return this;
                    }
    ...
    public void reloadList(List<Group> list) {
        clear();
//        addAll(list);
        for (Group c:list) {
            add(c);
