...
public class ContactsAdapter extends BaseSectionsAdapter {
    ...
    private boolean onlyOnlines=false;
    ...
    public Object getItem(int section, int position) {
        ...
        if(onlyOnlines) {
            usersSectionsDict = ContactsController.getInstance().onlineUsersSectionsDict;
            sortedUsersSectionsArray = ContactsController.getInstance().onlineSortedUsersSectionsArray;
        }else {
...
//        ArrayList<String> sortedUsersSectionsArray = onlyUsers == 2 ? ContactsController.getInstance().sortedUsersMutualSectionsArray : ContactsController.getInstance().sortedUsersSectionsArray;
    ...
    public View getItemView(int section, int position, View convertView, ViewGroup parent) {
        ...
        if (type == 4) {
            ...
            if (convertView == null) {
                GreySectionCell greySectionCell = new GreySectionCell(mContext);
                Switch onlyOnlinesCheckBox = new Switch(mContext);
                onlyOnlinesCheckBox.setClickable(true);
                onlyOnlinesCheckBox.setText(LocaleController.getString("OnlinesOnly", R.string.onlines_only));
                onlyOnlinesCheckBox.setGravity(Gravity.CENTER_VERTICAL);
                onlyOnlinesCheckBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                    @Override
                    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                        onlyOnlines=isChecked;
                        reloadList();
                        if(isChecked){
                            GoogleAnalyticsHelper.sendEventAction(mContext, Constant.Analytic.ONLY_ONLINE_USERS_EVENT_NAME);
                        }
                    }
                });
                greySectionCell.addView(onlyOnlinesCheckBox, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.MATCH_PARENT, (LocaleController.isRTL?Gravity.LEFT:Gravity.RIGHT) | Gravity.CENTER_VERTICAL, 40, 0, 40, 0));
                greySectionCell.setText(LocaleController.getString("Contacts", R.string.Contacts).toUpperCase());
                greySectionCell.setClickable(true);
                convertView=greySectionCell;
            }
        } else if (type == 2) {
            if (convertView == null) {
