...
public class SepehrPasscodeView extends FrameLayout {
    ...
    public SepehrPasscodeView(final Context context) {
        super(context);
    ...
    private void processDone(boolean fingerprint) {
        if (!fingerprint) {
            ...
            if (!UserConfig.checkSepehrPasscode(password)) {
                passwordEditText.setText("");
                passwordEditText2.eraseAllCharacters(true);
                onPasscodeError();
                return;
        ...
        if (Build.VERSION.SDK_INT >= 14) {
            ...
            animatorSetProxy.addListener(new AnimatorListenerAdapterProxy() {
                @Override
                public void onAnimationEnd(Object animation) {
                    SepehrPasscodeView.this.clearAnimation();
