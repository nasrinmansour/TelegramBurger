...
public class ChatBaseCell extends BaseCell implements MediaController.FileDownloadProgressListener {
    ...
    private int share2StartX;
    private int share2StartY;
    private boolean share2Pressed;
    ...
    public interface ChatBaseCellDelegate {
        ...
        void didPressShare2(ChatBaseCell cell);
    ...
    public ChatBaseCell(Context context) {
        ...
        if (timePaint == null) {
            ...
            replyNamePaint.setTypeface(AndroidUtilities.getTypeface(AndroidUtilities.getFont1Path()));
        ...
        this.statusBG.setCornerRadius(AndroidUtilities.dp(12.0F));
        this.statusBG.setStroke(AndroidUtilities.dp(1.0F), -1);
    }
        ...
        if (authorName || viaBot) {
            ...
            if (viaBot) {
                ...
                if (currentNameString.length() > 0) {
                    ...
                    stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface(AndroidUtilities.getFont1Path()), 0, currentMessageObject.isOutOwner() ? 0xff4a923c : 0xff006fc8), nameStringFinal.length() + 5, stringBuilder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    ...
                    stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface(AndroidUtilities.getFont1Path()), 0, currentMessageObject.isOutOwner() ? 0xff4a923c : 0xff006fc8), 4, stringBuilder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    nameStringFinal = stringBuilder;
                }
            }
            try {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            if (delegate == null || delegate.canPerformActions()) {
                if (isAvatarVisible && avatarImage.isInsideImage(x, y)) {
                    ...
                    replyPressed = true;
                    result = true;
                }  else if (drawShareButton && x >= share2StartX && x <= share2StartX + AndroidUtilities.dp(40) && y >= share2StartY && y <= share2StartY + AndroidUtilities.dp(32)) {
                    share2Pressed = true;
                    result = true;
                    invalidate();
            ...
            if (avatarPressed) {
                ...
                invalidate();
            }else if (share2Pressed) {
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    share2Pressed = false;
                    playSoundEffect(SoundEffectConstants.CLICK);
                    if (delegate != null) {
                        delegate.didPressShare2(this);
                    }
                } else if (event.getAction() == MotionEvent.ACTION_CANCEL) {
                    share2Pressed = false;
                } else if (event.getAction() == MotionEvent.ACTION_MOVE) {
                    if (!(x >= share2StartX && x <= share2StartX + AndroidUtilities.dp(40) && y >= share2StartY && y <= share2StartY + AndroidUtilities.dp(32))) {
                        sharePressed = false;
                    }
                }
                invalidate();
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (changed || !wasLayout) {
            ...
            if (isAvatarVisible) {
                ...
                setDrawableBounds(this.statusBG,  AndroidUtilities.dp(40.0F),layoutHeight - AndroidUtilities.dp(19.0f), AndroidUtilities.dp(12.0F), AndroidUtilities.dp(12.0F));
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isAvatarVisible) {
            ...
            if (this.currentUser != null && this.drawStatus && !currentUser.bot) {
        ...
        if (drawShareButton) {
            ...
            // draw sepehr forward button
            ResourceLoader.shareDirectDrawable[ApplicationLoader.isCustomTheme() ? 1 : 0][share2Pressed ? 1 : 0].setBounds(share2StartX = currentBackgroundDrawable.getBounds().right + AndroidUtilities.dp(8), share2StartY = layoutHeight - AndroidUtilities.dp(85), currentBackgroundDrawable.getBounds().right + AndroidUtilities.dp(40), layoutHeight - AndroidUtilities.dp(53));
            ResourceLoader.shareDirectDrawable[ApplicationLoader.isCustomTheme() ? 1 : 0][share2Pressed ? 1 : 0].draw(canvas);
        }
    ...
    private void setStatusColor(){
        if (this.currentUser != null && this.drawStatus && !currentUser.bot) {
            String str = LocaleController.formatUserStatus(this.currentUser);
            this.lastStatus = str;
