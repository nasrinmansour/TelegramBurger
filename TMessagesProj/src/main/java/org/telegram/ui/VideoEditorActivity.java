...
import org.telegram.ui.Components.VideoEditorFrameLayoutDrawer;
...
public class VideoEditorActivity extends BaseFragment implements TextureView.SurfaceTextureListener, NotificationCenter.NotificationCenterDelegate {
    ...
    private int menu_caption=2;
    private int menu_done=1;
    private int menu_caption_done=3;
                ...
                if (id == -1) {
                    ...
                    if (delegate != null) {
                        if (compressVideo.getVisibility() == View.GONE || compressVideo.getVisibility() == View.VISIBLE && !compressVideo.isChecked()) {
                            delegate.didFinishEditVideo(videoPath, startTime, endTime, originalWidth, originalHeight, rotationValue, originalWidth, originalHeight, originalBitrate, estimatedSize, esimatedDuration, captionTextView.getText().length() > 0 ? captionTextView.getText().toString() : null);
                        } else {
                            delegate.didFinishEditVideo(videoPath, startTime, endTime, resultWidth, resultHeight, rotationValue, originalWidth, originalHeight, bitrate, estimatedSize, esimatedDuration,captionTextView.getText().length() > 0 ? captionTextView.getText().toString() : null

                            );
                        }
                    }
                    finishFragment();
                } else if (id == menu_caption) {
...
//                    }
        ...
        captionDoneItem = menu.addItemWithWidth(menu_caption_done, R.drawable.ic_done, AndroidUtilities.dp(56));
        captionItem = menu.addItemWithWidth(menu_caption, R.drawable.photo_text, AndroidUtilities.dp(56));
        doneItem = menu.addItemWithWidth(menu_done, R.drawable.ic_done, AndroidUtilities.dp(56));
        ...
        captionTextView = new TextView(getParentActivity());
        ...
        captionTextView.setGravity(Gravity.CENTER_VERTICAL);
        captionTextView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 16);
        captionTextView.setVisibility(View.GONE);
        ((ViewGroup) this.fragmentView).addView(captionTextView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT, Gravity.TOP | Gravity.LEFT, 0.0F, 10.0F, 0.0F, 48F));

        captionEditText = new PhotoViewerCaptionEnterView(getParentActivity(), ((VideoEditorFrameLayoutDrawer)fragmentView));
        ...
        ((VideoEditorFrameLayoutDrawer)fragmentView).setCaptionEditText(captionEditText,new Runnable(){
            @Override
            public void run() {
                closeCaptionEnter();
            }
        });
        ((ViewGroup)fragmentView).addView(captionEditText, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT, Gravity.BOTTOM | Gravity.LEFT, 0, 0, 0, 0));
    ...
    private void closeCaptionEnter() {
//        Object object = imagesArrLocals.get(currentIndex);
//        if (apply) {
//            if (object instanceof MediaController.PhotoEntry) {
//                ((MediaController.PhotoEntry) objectobject).caption = captionEditText.getFieldCharSequence();
...
//            captionItem.setIcon(2130838119);
            captionTextView.setTag(caption);
            captionTextView.setText(caption);
            captionTextView.setVisibility(View.VISIBLE);
            ViewProxy.setAlpha(captionTextView, 1.0F);
        }else {
//        this.captionItem.setIcon(2130838118);
